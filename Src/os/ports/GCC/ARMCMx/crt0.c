...
/**
 * @brief   RAMCODE segment initialization switch.
 */
#if !defined(CRT0_INIT_RAMCODE) || defined(__DOXYGEN__)
#define CRT0_INIT_RAMCODE               TRUE
#endif
...
/**
 * @brief   Ramcode segment start.
 * @pre     The symbol must be aligned to a 32 bits boundary.
 */
extern uint32_t _ramcode_start;

/**
 * @brief   Ramcode segment end.
 * @pre     The symbol must be aligned to a 32 bits boundary.
 */
extern uint32_t _ramcode_end;

/**
 * @brief   ROM image of the ramcode segment start.
 * @pre     The symbol must be aligned to a 32 bits boundary.
 */
extern uint32_t _textramcode;
...
#if !defined(__DOXYGEN__)
__attribute__((naked))
__attribute__ (( section(".resetHandlerSection") ))
#endif
void ResetHandler(void)
{
  uint32_t psp, reg;
 *((uint32_t*)0xE000ED08) = (uint32_t)&_vectors;
...
#ifdef CRT0_INIT_RAMCODE
  	/* RAMCODE segment initialization.*/
	{
		uint32_t *tp, *dp;

		tp = &_textramcode;
		dp = &_ramcode_start;
		while (dp < &_ramcode_end)
			*dp++ = *tp++;
	}
#endif
